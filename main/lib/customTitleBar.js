...
/**
 * Зачем нужен customTitleBar?
 * В BrowserWindows мы скрываем нативный заголовок приложения с его кнопками управления окна.
 * В MacOS по умолчанию показывается светофор, а вот в Windows кнопки пропадают.
 * В домене уже реализован компонент TitleBar, который для нашего приложения отрисовывает нужные кнопка.
 * Чтобы пользователь на странице авторизации (паспорта) мог закрыть окно, то мы туда встраиваем наш кастомный TitleBar.
 *
 * Этот customTitleBar утратит актуальность после доработки авторизационного флоу.
 */
